...
export default function MasterDataTab() {
  ...
  // Scroll to and clear recently added highlight
  useEffect(() => {
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        title="Master Data"
        subtitle={`Editable template — ${apis.length} APIs across ${rows.length} stages. Yellow cells are inputs that recompute the entire schedule.`}
        right={
      ...
      {showForm && (
        <AddStageForm
          ...
          onAdded={() => {
            setShowForm(false);
            // small delay so the row exists when we scroll
          }}
        />
      )}
          ...
          <table className="min-w-full text-sm">
            <thead className="sticky top-0 z-10 bg-ink-900/90 backdrop-blur-md">
              <tr className="text-left text-[11px] uppercase tracking-wider text-ink-300">
                ...
                <Th align="right">Output (kg)</Th>
                <Th align="right">&nbsp;</Th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => {
                const output = r.batchSizeKg * r.plannedBatches;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <Tag tone={r.stageName === "Final API" ? "violet" : "cyan"}>
                        <Beaker size={10} /> {r.stageName}
                      </Tag>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <EditableCell
                        value={r.plannedBatches}
                        onChange={(v) =>
                          updateStageField(r.id, "plannedBatches", v)
                        }
                      />
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono font-semibold text-cyan-300 tabular-nums">
              ...
              {rows.length === 0 && (
                <tr>
                  ...
                  >
                    No stages match. Click <span className="font-bold text-cyan-300">+ Add Stage</span> above to create one.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="rounded-xl border border-amber-300/20 bg-amber-300/5 p-3 text-xs text-amber-200">
        <span className="mr-1 font-bold">
          <Pencil size={12} className="inline" /> Tip:
        </span>
        Edit any yellow cell, click <span className="font-bold text-cyan-300">+ Add Stage</span> to extend an API or create a new one,
        or hover a row and click the trash icon to remove it. The scheduler re-runs in ~350 ms and every other tab updates live.
      </div>
    </div>
  );
}
