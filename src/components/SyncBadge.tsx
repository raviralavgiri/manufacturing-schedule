...
export default function SyncBadge() {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="popover-surface absolute right-0 top-full z-50 mt-2 w-[160px] overflow-hidden rounded-xl shadow-2xl shadow-black/60 ring-1 ring-cyan-300/20">
          <div className="space-y-1.5 p-2">
            ...
            <ModeRow
              ...
              title="L"
              fullName="Local (Browser)"
              icon={<HardDrive size={14} />}
            />
          </div>

          {/* Conditional warnings only — kept because they signal real
              problems (e.g., Supabase mis-configured). */}
          {!isSupabaseEnabled && (
            <div className="mx-2 mb-2 flex items-start gap-2 rounded-md border border-amber-300/30 bg-amber-300/5 p-2 text-[10px] text-amber-200">
              <ShieldAlert size={12} className="mt-0.5 shrink-0" />
              <div>Cloud unavailable.</div>
            </div>
          )}

          {cloudError && (
            <div className="mx-2 mb-2 flex items-start gap-2 rounded-md border border-rose-300/30 bg-rose-400/10 p-2 text-[10px] text-rose-200">
          ...
          {opMessage && (
            <div
              className={clsx(
                "mx-2 mb-2 flex items-start gap-2 rounded-md border p-2 text-[10px]",
                opMessage.tone === "ok"
...
function ModeRow({
  ...
  disabled?: boolean;
  onSelect: () => void;
  /** Compact button label, e.g. "CL" / "L". */
  title: string;
  /** Hover tooltip with the full descriptive name. */
  fullName: string;
  ...
  return (
    ...
    >
      ...
      <span
        className={clsx(
          "flex-1 text-[12px] font-extrabold tracking-wider",
